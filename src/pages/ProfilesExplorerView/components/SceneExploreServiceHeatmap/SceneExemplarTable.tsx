...
export class SceneExemplarTable extends SceneObjectBase<SceneExemplarTableState> {
  ...
  onActivate() {
    ...
    const stateSub = this.subscribeToState((newState, prevState) => {
      if (newState.tempoDataSourceUid && newState.tempoDataSourceUid !== prevState.tempoDataSourceUid) {
        const pending = newState.rows
          .filter((r) => r.spanId && !(r.spanId in newState.traceInfoBySpanId))
          .map((r) => r.spanId!);
        const unique = [...new Set(pending)];
        if (unique.length > 0) {
          this.lookupTraceInfo(unique);
        }
      }
    });
  ...
  static Component({ model }: SceneComponentProps<SceneExemplarTable>) {
    ...
    try {
      ...
      selectedSpanId = parent.useState().selectedSpanId;
    } catch {
    ...
    return (
      <div className={styles.container}>
        <div className={styles.header}>
          <span className={styles.title}>Top span exemplars</span>
          <div className={styles.picker}>
            <span className={styles.pickerLabel}>Tempo datasource</span>
            <Select
              options={dsOptions}
              value={tempoDataSourceUid ?? null}
              onChange={(option) => {
                model.setState({ tempoDataSourceUid: option?.value ?? undefined });
              }}
        ...
        {rows.length === 0 ? (
          <div className={styles.empty}>No span exemplars in the selected time range.</div>
        ) : (
